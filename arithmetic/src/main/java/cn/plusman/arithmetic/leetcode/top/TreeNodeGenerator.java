...
public class TreeNodeGenerator {
    ...
    public static Node fromListToNode(List<Integer> arr) {
        if (arr.isEmpty()) {
            return null;
        }
        ...
        Map<Integer, List<Node>> horizontal = new HashMap<>();
        
        int count = 0;
        for (int i = 0;; i++) {
            double capacity = Math.pow(2, i);
            List<Node> line = horizontal.computeIfAbsent(i, (k) -> new ArrayList<>());
            ...
            for (int j = 0; j < capacity; j++) {
                ...
                if (i == 0) {
                    ...
                    Node parent = horizontal.get(i - 1).get(j / 2);
                    if (j % 2 == 0) {
                        parent.left = treeNode;
                    } else {
                        parent.right = treeNode;
